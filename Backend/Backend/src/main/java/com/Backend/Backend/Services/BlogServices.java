...
public class BlogServices implements IBlogServices {
    ...
    public void blogCreation(BlogCreationDto blogCreationDto) {
        blogMapper.toBlogResponseDto(blogRepository.save(blogMapper.toBlog(blogCreationDto)));
    }
    ...
    public BlogResponseDto updateBlog(Long blogId, BlogRequestDto blogRequestDto) {
        ...
        ParentUser user = parentUserRepository.findById(blogRequestDto.getBlogger().getId())
                .orElseThrow(() -> new RuntimeException("User with id:" + blogRequestDto.getBlogger().getId() +"not found :-("));
